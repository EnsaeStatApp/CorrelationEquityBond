...
class GenericHMMBacktester:
    ...
    def get_predicted_corrs_df(self) -> pd.Series:
        """
        Extracts the predicted (weighted) correlation between the first two assets.
        
        Returns:
            pd.Series: Time-series of predicted correlations.
        """
        dates = sorted(self.results.keys())
        corrs = []
        for d in dates:
            ...
            # Correlation formula: cov(i,j) / (std(i) * std(j))
            rho = cov[0, 1] / (vols[0] * vols[1] + 1e-15)
            corrs.append(rho)
        
        return pd.Series(corrs, index=dates, name="Predicted_Correlation")

    def plot_predicted_risk_metrics(self, figsize: tuple = (14, 8)):
        """
        Plots predicted volatilities and correlation in a dual-subplot layout.
        Assumes the backtester has already been run.
        """

        # 1. Prepare Data
        vol_df = self.get_risk_df()
        corr_series = self.get_predicted_corrs_df()
        
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=figsize, sharex=True)

        # --- Subplot 1: Annualized Volatilities ---
        for col in vol_df.columns:
            ax1.plot(vol_df.index, vol_df[col], label=f"Pred. {col}", lw=1.8)
        
        ax1.set_title("Predicted Annualized Volatilities (Weighted by Regime Probs)", fontsize=14, fontweight='bold')
        ax1.set_ylabel("Volatility (Ann.)", fontsize=12)
        ax1.legend(fontsize=11, loc='upper left')
        ax1.grid(alpha=0.3)
        ax1.tick_params(axis='y', labelsize=11)

        # --- Subplot 2: Correlation ---
        ax2.plot(corr_series.index, corr_series, color='red', lw=2, label="Stock-Bond Correlation")
        ax2.axhline(0, color='black', lw=1, ls='--') # Zero line for correlation
        
        ax2.set_title(f"Predicted Correlation: {self.asset_vars[0]} / {self.asset_vars[1]}", fontsize=14, fontweight='bold')
